...
func _physics_process(delta):	
	var inp_x := Input.get_axis("game_left", "game_right") if not input_locked else 0.0
	var inp_y := Input.get_axis("game_up", "game_down") if not input_locked else 0.0
	var grav := 0.98
